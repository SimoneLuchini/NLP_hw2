...
INPUT_FOLDER = '/Users/simone/desktop/Homework2_NLP/LSTM_task'
OUTPUT_FOLDER = '/Users/simone/desktop/Homework2_NLP/LSTM_task'
...
w2vmodel = Word2Vec.load(OUTPUT_FOLDER + '/models/' + 'word2vec_500_PAD.model')
...
class LSTMTextClassifier(nn.Module):
    def __init__(self, vocab_size, num_classes, hidden_size=128, num_layers=2, bidirectional=True):
    ...
    def forward(self, x):
        ...
        out = torch.cat((out[:, -1, :self.lstm.hidden_size], out[:, 0, self.lstm.hidden_size:]), dim=1) if self.lstm.bidirectional else out[:, -1, :]
        logits = self.fc(out)
...
loss_file_name = OUTPUT_FOLDER + '/plots/' + 'lstm_class_big_loss_with_padding.csv'
...
for epoch in range(num_epochs):
    ...
    for index, row in progress_bar:
        ...
        # Make the sequence of word embeddings for stemmed tokens
        word_embeddings = make_word2vec_vector_lstm(row['stemmed_tokens'])

        # Forward pass to get output
        probs = lstm_model(word_embeddings)
...
##########
#check hidden layer
lstm_model = torch.load(OUTPUT_FOLDER + '/lstm_big_model_500_with_padding.pth', map_location=torch.device('cpu'))

embedding_params = sum(p.numel() for p in lstm_model.embedding.parameters())
print(f"Number of parameters in the embedding layer: {embedding_params}")
